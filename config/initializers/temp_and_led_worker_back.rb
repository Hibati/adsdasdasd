...
Channel.all.each do |channels|
    ...
    apikey = ApiKey.where(channel_id: channels.id, write_flag: 1).first.api_key
    ...
    if actionstatus == "running" && when_this_stuff_happens == "Temperature" && then_do_these_things_1st == "Led"
      $cron_thread_talwb << Thread.new do
